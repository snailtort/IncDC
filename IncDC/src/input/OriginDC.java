	...
	public OriginDC(File hydra, ParsedColumn<?> [] col){
        ...
        try{
            ...
            while((s = br.readLine())!=null){
                ...
                list=s;
				if(list.contains("DC")) list=list.substring(list.indexOf("this is")+8);
				else list = list.substring(list.indexOf("this is")+6);

                if(list.contains("this is ")) s=list.substring(list.indexOf("["),list.indexOf("this is"));
                else s=list.substring(list.indexOf("[")+7);
				...
				while(s.contains(")") || s.contains("]")) {
					...
					if(s.contains(AND)) s=s.substring(s.indexOf(AND)+1);
	    			else s = "-1";
					boolean flag1=false;
					boolean flag2=false;
